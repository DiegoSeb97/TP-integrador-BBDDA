...
@APELLIDO VARCHAR(20), @DNI INT, @DIRECCION VARCHAR(100), @MAILPERSONAL VARCHAR(50), 
	...
	BEGIN
		...
		BEGIN
			...
			BEGIN
				...
				CARGO = @CARGO, SUCURSAL = @SUCURSALID, TURNO = @TURNO WHERE DNI = @DNI;
			END
		END
	END
END
...
BEGIN
	IF (SELECT BAJA FROM SUCURSAL.SUCURSAL WHERE ID = (SELECT SUCURSAL FROM SUCURSAL.EMPLEADO WHERE LEGAJOID = @LEGAJO)) IS NOT NULL
...
BEGIN
	...
	IF NOT EXISTS (SELECT ID FROM CATALOGO.PRODUCTO WHERE NOMBRE = @NOMBRE)
		INSERT INTO CATALOGO.PRODUCTO(CATEGORIA, NOMBRE, PRECIO, PRECIO_REFERENCIA, CANTPORUNIDAD,
...
--INSERTAR VENTA
CREATE OR ALTER PROCEDURE VENTASSUCURSAL.REGISTRAR_VENTA (@CIUDAD VARCHAR(20), @FECHA DATE, @HORA TIME, @EMPLEADO_ID INT, @ID_FACT CHAR(11), @MEDIO_PAGO VARCHAR(20)) AS
BEGIN
	...
	BEGIN
		IF NOT EXISTS (SELECT 1 FROM ventasSucursal.venta_registrada WHERE id_FACTURA = @ID)
			INSERT INTO ventasSucursal.venta_registrada(ID_FACTURA, CIUDAD, FECHA, HORA, EMPLEADO_ID, MEDIO_PAGO)
			VALUES (@ID, @CIUDAD, @FECHA, @HORA, @EMPLEADO_ID, @ID_PAGO)
		ELSE
			UPDATE ventasSucursal.venta_registrada SET CIUDAD = @CIUDAD, FECHA = @FECHA, HORA = @HORA, MEDIO_PAGO = @ID_PAGO, empleado_id = @EMPLEADO_ID WHERE id_factura = @ID
	END
END
go
